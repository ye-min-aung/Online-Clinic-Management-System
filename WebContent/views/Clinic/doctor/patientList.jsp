...
<head>
  ...
  <title>Doctor(Schedule Patient)</title>
    <style>
...
<body>
        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse justify-content-end menulist bBorder" id="collapsibleNavbar" style="padding-right:130px;font-family: 'Asar';">
            <ul class="navbar-nav ">
              ...
              <li class="nav-item">
                <a class="nav-link text-dark address" href="${pageContext.request.contextPath}/views/Clinic/doctor/patientList.jsp">SCHEDULE PATIENT</a>
              </li>  
  ...
  <div class="hero-section">
         ...
         <center><h2>Patient Details</h2></center><br>
         <div class="d-flex justify-content-center">   
        <form action="/ClinicManagementSystem/schedulePatient" method="post" class="d-flex " style="float:left">
           <input class="form-control me-2" type="date" placeholder="Search" name="searchName" aria-label="Search" style="width: 300px;">
           <button class="btn btn-success  me-3" type="submit" >Search</button>
       </form>
       <form action="/ClinicManagementSystem/schedulePatient" method="get" style="float:left">
           <button class="btn btn-success  " type="submit" >View All</button>
       </form>
    </div>
    <br><br>
        <table class="table table-bordered ms-4 " style="width:96%">
          <thead class="table-primary text-center">
            <tr>
              ...
              <th scope="col">Reception Name</th>
          ...
          <%
				...
				ArrayList<Appointment> appInfo = (ArrayList<Appointment>)ss.getAttribute("appointment");

				 for(Appointment a : appInfo){
			%>
          <tbody class="text-center">
              <tr>
                <td> <%=a.getAppointment_id()%> </td>
				<td> <%=a.getPatient_name()%> </td>
				<td> <%=a.getAppointment_date()%> </td>
				<td> <%=a.getAppointment_time()%></td>
				<td> <%=a.getReception_name()%> </td>
				<td> <%=a.getStatus()%> </td>
				 <td>
                  <form action="/ClinicManagementSystem/Doctor/doctorMedicine" method="POST" style="display: inline;">
                  <input type="hidden" name="appointment_id" value="<%=a.getAppointment_id()%>">
                  <input type="hidden" name="prescription_date" value="<%=a.getAppointment_date()%>">
                    <button type="submit" class="btn btn-success btn-sm" >Prescription</button>
                  </form>
                  <form action="/ClinicManagementSystem/views/Clinic/doctor/MedicalRecord.jsp" method="POST" style="display: inline;">
                    <input type="hidden" name="appointment_id" value="<%=a.getAppointment_id()%>">
              		<input type="hidden" name="visit_date" value="<%=a.getAppointment_date()%>">
              		<input type="hidden" name="patient_id" value="<%=a.getPatient_id()%>">       
                    <button type="submit" class="btn btn-success btn-sm" >Medical Record</button>
                  </form>
                
              
                  <form action="/ClinicManagementSystem/Doctor/patientDetails" method="post" style="display: inline;">
                    <input type="hidden" name="appointment_id" value="<%=a.getAppointment_id()%>">
                    <input type="hidden" name="patient_id" value="<%=a.getPatient_id()%>">
                    <button type="submit" class="btn btn-success btn-sm " style="" >Patient Details</button>
                  </form>
                </td>
