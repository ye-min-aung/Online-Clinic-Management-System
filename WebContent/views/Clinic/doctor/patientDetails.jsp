...
<head>
  ...
  <title>Doctor(Patient Details & View Medical Record)</title>
    <style>
...
<body>
  ...
  <div class="hero-section">
        ...
        <table class="table table-bordered ms-4 " style="width:95%;">
          ...
          <%
				Patient patient = (Patient) session.getAttribute("patientDetails");
          %>
          <tbody class="text-center">
            <tr>
              ...
              <td><%= patient.getPatient_gender() %></td>
              <td><%= patient.getPatient_birth_date() %></td>
    ...
    <%
        // Retrieve the medical records from the session
        ArrayList<MedicalRecord> medicalRecords = (ArrayList<MedicalRecord>) session.getAttribute("medicalRecords");
    %>
     <br><br><center><h2>Medical Records</h2></center><br>
    <table border="1" class="table table-bordered ms-4 " style="width:95%;">
        ...
        <%
            // Loop through the medical records and display each one in a table row
            for (MedicalRecord medicalRecord : medicalRecords) {
        %>
            <tr>
                <td><%= medicalRecord.getRecordId() %></td>
                <td><%= medicalRecord.getPatientId() %></td>
                <td><%= medicalRecord.getDoctorId() %></td>
                <td><%= medicalRecord.getVisitDate() %></td>
                <td><%= medicalRecord.getDiagnosis() %></td>
                <td><%= medicalRecord.getTreatment() %></td>
                <td><%= medicalRecord.getFollowUpDate() %></td>
                <td><%= medicalRecord.getAllergic() %></td>
            </tr>
        <%
            }
        %>
