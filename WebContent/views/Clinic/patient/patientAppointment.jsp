...
<a class="nav-link text-dark address" href="/ClinicManagementSystem/Patient/patientDoctorList">DOCTOR LIST</a>
            ...
            <form action="/ClinicManagementSystem/Appoinment" method="get">
        		<input type="hidden" name="patient_id" value="<%= patientId%>">
              <button  id="App" type="submit" style="border:none;background-color: rgba(43, 187, 240, 0.02);">
              	APPOINTMENT
              </button>
            </form>
            </a>
...
<div class="hero-section">
   <div class="container-fluid">
 <center> <H3 >Appointment Status</H3></center><br>
    	...
    	<%
				...
				ArrayList<Appointment> UserInfo = (ArrayList<Appointment>)ss.getAttribute("UserAppointment");
				%>
		<%if(UserInfo != null){ %>
		 <table>
			...
			<tr>
				...
				<td> <%=u.getDoctor_specialization()%> </td>
				<td> <%=u.getStatus()%> </td>
				<td> <%=u.getReception_name()%> </td>	
			</tr>
				<%
					}
				%>
   			...
   			<%}else{ %>
   			<div class="alert alert-success" role="alert">You don't have any appointment.</div>
   			<%} %>
			
			<%
				HttpSession sss = request.getSession();

				ArrayList<Appointment> FailInfo = (ArrayList<Appointment>)sss.getAttribute("deleteList");

			%>
			<% if(FailInfo != null){
				
				for(Appointment f: FailInfo){%>
		   <br> <div class="alert alert-success" role="alert">Your booking for  <%=f.getDoctor_name()%> on <%=f.getAppointment_date()%>  at  <%=f.getAppointment_time()%>  is not available.;
</div>
		    
		    
			<%}
				sss.removeValue("deleteList");} %>
