...
<head>
    ...
    <title>Admin(View Reception)</title>
<style>
	table {
            ...
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0 me-5">
              ...
              <li class="nav-item dropdown">
                ...
                <ul class="dropdown-menu text-dark">
                  ...
                  <li><a class="dropdown-item bg-white text-primary" href="${pageContext.request.contextPath}/views/Clinic/admin/ViewPatient.jsp">View Patient</a></li>
                </ul>
              </li>
...
%>
 ...
 <center><h3>Reception Information</h3></center><br>
       <div class="d-flex justify-content-center">
       <form action="/ClinicManagementSystem/adminViewReceptionList" method="post" class="d-flex " style="float:left">
           <input class="form-control me-2" type="search" placeholder="Search Receptions" name="searchName" aria-label="Search" style="width: 300px;">
           <button class="btn btn-success me-3" type="submit">Search</button>
       </form>
       <form action="/ClinicManagementSystem/adminViewReceptionList" method="get" style="float:left" >
  ...
  <table class="m-3 ">
			...
			ArrayList<Reception> receptionInfo = (ArrayList<Reception> )ss.getAttribute("receptionList");

				 for(Reception r: receptionInfo){
			%>

			<tr>
				<td> <%=r.getReception_id()%> </td>
				<td> <%=r.getReception_name()%> </td>
				<td> <%=r.getReception_password() %> </td>
			    <td> <%=r.getReception_address()%> </td>
				<td> <%=r.getReception_gender()%> </td>
				<td> <%=r.getReception_birth_date()%> </td>
				<td> 
				 <form action="/ClinicManagementSystem/views/Clinic/admin/adminReceptionUpdate.jsp" style="display: inline;">
                   <input type="hidden" name="reception_Id" value="<%=r.getReception_id()%>">
                      <button type="submit" class="btn btn-success btn-sm" name="receptionUpdate">Update</button>
                    </form>
                    <form action="/ClinicManagementSystem/admin/receptiondelete" method="POST" style="display: inline;">
                    <input type="hidden" name="reception_id" value="<%=r.getReception_id() %>" >
                      <button type="submit" class="btn btn-danger btn-sm" name="receptionDelete">Delete</button>
                    </form>
                </td>
                <td>
					<% if (r.getIs_confirmed() == 0) { %>
						<form action="/ClinicManagementSystem/admin/confirmReceptionAccount" method="post" style="display: inline;">
							<input type="hidden" name="reception_id" value="<%=r.getReception_id() %>" >
